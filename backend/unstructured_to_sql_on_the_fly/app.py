...
# ─── Setup ──────────────────────────────────────────────────────────────────────
dotenv.load_dotenv()
pw.set_license_key("demo-license-key-with-telemetry")  # comment out if using Community
logging.basicConfig(level=logging.INFO)

# ─── Schema ────────────────────────────────────────────────────────────────────
class IncidentReportSchema(pw.Schema):
    tracking_id: str
    incident_id: str
    driver_id: str
    type: str
...
@pw.udf
def strip_metadata(docs: list[tuple[str, dict]]) -> list[str]:
    return [text for text, _ in docs]

@pw.udf
def build_prompt(texts: list[str]) -> str:
    docs_str = " ".join(texts)
    enc = tiktoken.get_encoding("cl100k_base")
    prefix = "You are given an unstructured incident report PDF. Extract these fields:\n"
    suffix = (
        f"\nReturn JSON matching this schema {IncidentReportSchema.typehints()}\n"
        "Rules:\n"
        "- report_date in YYYY-MM-DD\n"
        "- Include all fields (empty string if missing)\n"
        "- Output valid JSON\n"
    )
    # enforce token limit
    available = 8000 - (len(enc.encode_ordinary(prefix)) + len(enc.encode_ordinary(suffix)))
    toks = enc.encode_ordinary(docs_str)
    if len(toks) > available:
        logging.warning("Truncating document to fit prompt size")
        docs_str = enc.decode(toks[:available])
    return prefix + docs_str + suffix

@pw.udf
def parse_response(response: str) -> list[str]:
    data = json.loads(response)
    # return in sorted-schema order
    return [data[k] for k in sorted(IncidentReportSchema.keys())]

# ─── Main ───────────────────────────────────────────────────────────────────────
def run():
    data_dir = os.environ.get("PATHWAY_DATA_DIR", "./data/incident_report")
    ...
    docs = (
        files
        .select(texts=parser(pw.this.data))
        .select(texts=strip_metadata(pw.this.texts))
    )

    # 3) LLM → structured table
    prompts = docs.select(prompt=build_prompt(pw.this.texts))
    llm = OpenAIChat(
    ...
    raw = prompts.select(raw=llm(prompt_chat_single_qa(pw.this.prompt)))
    parsed = raw.select(values=parse_response(pw.this.raw))
    structured = unpack_col(parsed.values, *sorted(IncidentReportSchema.keys()))

    # 4) Write CSV
    csv_path = "./data/incident_reports.csv"
    pw.io.csv.write(structured, csv_path)

    # 5) Run the graph so the CSV is actually materialized
    pw.run(monitoring_level=pw.MonitoringLevel.NONE)

    # 6) Now copy to output (after pw.run completes)
    shutil.copy(csv_path, "./data/output/incident_reports.csv")
    logging.info(f"Copied {csv_path} → ./data/output/incident_reports.csv")
